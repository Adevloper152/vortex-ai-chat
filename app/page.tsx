...
export default function Home() {
  ...
  const handleSubmit = useCallback(
    async (text: string): Promise<void> => {
      ...
      if (text === '' && (!files || files.length === 0)) return // Prevent empty message submission
      ...
      if (temporaryChatMode) {
        setTemporaryMessages((prev) => [...prev, newUserMessage])
      } else {
        const { add: addMessage } = useMessageStore.getState()
        addMessage(newUserMessage)
      }
      let messages: Message[] = temporaryChatMode ? [...temporaryMessages, newUserMessage] : [...messagesRef.current]
      if (talkAudioMode) {
      ...
      const { summary } = useMessageStore.getState()
      if (summary.content !== '') {
  ...
  const handleKeyDown = useCallback(
    (ev: KeyboardEvent<HTMLTextAreaElement>) => {
      if ((ev.key === 'Enter' && (ev.metaKey || ev.ctrlKey)) || (ev.key === 'Enter' && ev.shiftKey && !isRecording) || (ev.key === 'Enter' && !ev.shiftKey && !isRecording)) {
        if (!checkAccessStatus()) return false
          ...
          {temporaryChatMode ? (
            <div className="w-full max-w-screen-md flex-1 overflow-y-auto scroll-smooth">
              <div className="flex grow flex-col justify-start">
                ...
                <div className="my-2 flex h-4 justify-center text-xs text-yellow-600 duration-300 dark:text-yellow-400">
                  <span>Temporary Chat Mode: messages are not saved.</span>
                </div>
